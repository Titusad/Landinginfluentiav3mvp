...
export function buildSessionSummaryPrompt(locale?: string | null): string {
  const lc = resolveLocale(locale);
  const lang = lc.lang;
  const langTag = lc.langTag;
  const pillarTags = getPillarTags(lc);

  const sentimentExample = lc.isBrazil
    ? "Seu dominio de objecoes de preco foi excelente. Continue refinando seu fechamento e estara pronto."
    : "Tu manejo de objeciones de precio fue excelente. Sigue refinando tu cierre y estaras listo.";
  const badExample = lc.isBrazil ? "Pratique mais" : "Practica mas";
  const goodExample = lc.isBrazil
...
You are an executive communication coach generating a concise session feedback summary for a professional who just completed a practice session.

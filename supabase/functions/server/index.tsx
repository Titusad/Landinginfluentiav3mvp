...
app.post("/make-server-08b8658d/prepare-session", async (c) => {
  try {
    const { systemPrompt, scenario, interlocutor, scenarioType, voiceId } = await c.req.json();
...
app.post("/make-server-08b8658d/process-turn", async (c) => {
  try {
    ...
    console.log(`[ProcessTurn] Session ${sessionId} | Turn ${session.turnCount} | User: "${userMessage.slice(0, 80)}..."`);
    ...
    console.log(`[ProcessTurn] ✅ Turn ${session.turnCount} complete | AI: ${aiText.length} chars | isComplete: ${isComplete}`);

    return c.json({ aiMessage, isComplete, coachingHint, _debug: { turnCount: session.turnCount, analysisCount: session.internalAnalysis.length } });
  } catch (err) {
...
app.post("/make-server-08b8658d/analyze-feedback", async (c) => {
  try {
    ...
    console.log(`[AnalyzeFeedback] Session ${sessionId} | ${messages.length} messages | ${internalAnalysis.length} analysis notes | type=${scenarioType || session.scenarioType}`);
    ...
    const analystSystemPrompt = buildFeedbackAnalystPrompt(effectiveScenarioType, fbLocale);

 ...
 */
export function buildFeedbackAnalystPrompt(): string {
  const lang = "Spanish";
  const langTag = "ES";
  ...
  const toneBad1 = "El usuario demostró competencia en...";

  const scarcityPhrase = "En los turnos disponibles, se observó...";
...
import { detectPronunciationMarket, type PronunciationMarket } from "../locale-detect";

/**
 * Build the pronunciation coach prompt for Gemini 1.5 Flash.
 *
 * Language-background specific phonetic directives are activated
 * automatically via silent browser locale detection (no user input needed).
 *
 ...
 * - session_context: { scenario, interlocutor }
 *
...
export function buildResultsSummaryPrompt(
  overrideMarket?: PronunciationMarket
): string {
  // Auto-detect from browser locale, or accept an explicit override (e.g., from server)
  const market = overrideMarket ?? detectPronunciationMarket();
...
${buildPronunciationCulturalDirective(market, "Claridad")}
...
${buildPronunciationCulturalDirective(market, "Ritmo")}
...
${buildPronunciationCulturalDirective(market, "Entonación")}
...
function buildPronunciationCulturalDirective(
  marketFocus: PronunciationMarket | null | undefined,
  category: string
): string {

...
app.post("/make-server-08b8658d/generate-script", async (c) => {
  try {
    ...
    const needsRewrite = wordCount < 350 || isFirstPerson || !hasContingency;

    if (needsRewrite) {
      console.log(`[Generate Script v3] Quality gates FAILED — triggering rewrite call. Reasons: ${wordCount < 350 ? "TOO_SHORT " : ""}${isFirstPerson ? "FIRST_PERSON " : ""}${!hasContingency ? "NO_CONTINGENCY" : ""}`);
...
${wordCount < 350 ? `❌ TOO SHORT: Currently ${wordCount} words. EXPAND every paragraph to 4-5 full sentences. Target: 450-600 words total.` : ""}
...
- Every paragraph must be 4-5 full sentences minimum
...
const ELEVEN_VOICE_ID = "21m00Tcm4TlvDq8ikWAM"; // Rachel — warm professional female
